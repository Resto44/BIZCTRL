...
const state = vi.hoisted(() => ({ client: null, tenantId: 'tenant-a', value: 120, role: 'owner', lang: 'en' }));
...
async function render() { await act(async () => root.render(<StrictMode><QueryClientProvider client={cache}><MemoryRouter initialEntries={['/inventory']}><BranchScopeProvider><RetailInventoryScopeProvider><Routes><Route element={<OwnerWorkspaceTabs />}>{INVENTORY_PAGES.map((p) => <Route key={p.key} path={p.path} element={<RetailInventory page={p.key} />} />)}</Route></Routes></RetailInventoryScopeProvider></BranchScopeProvider></MemoryRouter></QueryClientProvider></StrictMode>)); await act(async () => vi.advanceTimersByTimeAsync(2)); }
beforeEach(() => {
  vi.useFakeTimers(); localStorage.clear(); state.tenantId = 'tenant-a'; state.value = 120; state.role = 'owner'; state.lang = 'en';
