...
function DisplaySession({ token, rpc = cashierRpc, brand = 'RETAIL' }) {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      busy = true;
      try {
        const result = await rpc('customer_display', { p_token: token }, client);
  ...
  return <main dir={lang === 'ar' || lang === 'fa' ? 'rtl' : 'ltr'} className="min-h-dvh bg-[#f3f6fb] p-4 text-slate-950 sm:p-8 lg:p-12">
    <div className="mx-auto max-w-[1550px]">
      <header className="mb-8 flex flex-wrap items-center justify-between gap-4"><div className="flex items-center gap-3"><span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-600 text-white"><ShoppingBasket size={30} /></span><div><p className="text-xs font-bold tracking-[.18em] text-blue-600">BIZCTRL · {brand}</p><h1 className="text-2xl font-black sm:text-3xl">{data?.business_name || c.customerTitle}</h1><p className="text-sm text-slate-500">{data?.branch_name} {data?.device_code ? `· ${data.device_code}` : ''}</p></div></div><span className={`rounded-full px-4 py-2 text-sm font-bold ${visible ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-900'}`}>{visible ? c.updated : c.stale}</span></header>
...
export default function CustomerDisplay({ rpc = cashierRpc, brand = 'RETAIL' }) {
  const { hash } = useLocation();
  const token = hash.slice(1);
  return <DisplaySession key={`${brand}:${token}`} token={token} rpc={rpc} brand={brand} />;
}
