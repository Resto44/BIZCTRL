...
CREATE POLICY payment_proofs_owner_upload ON storage.objects FOR INSERT TO authenticated
WITH CHECK (
...
USING (
  bucket_id = 'payment-proofs'
  AND ((storage.foldername(name))[1] = auth.uid()::text OR public.platform_owner_is_authorized())
);
...
DECLARE
  ...
  v_subscription public.subscriptions; v_plan public.subscription_plans; v_status text;
  v_usage jsonb := '{}'::jsonb; v_pending_payment_id uuid; v_can_manage_billing boolean := false;
BEGIN
  IF auth.uid() IS NULL OR v_restaurant_id IS NULL THEN RETURN jsonb_build_object('found', false, 'has_erp_access', false, 'status', 'EXPIRED', 'test_mode_enabled', false, 'can_manage_billing', false); END IF;
  IF NOT EXISTS (SELECT 1 FROM public.erp_memberships membership WHERE membership.user_id = auth.uid() AND membership.status = 'approved' AND membership.restaurant_id = v_restaurant_id) THEN RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'SUBSCRIPTION_SCOPE_DENIED'; END IF;
  v_can_manage_billing := public.erp_is_approved_owner(v_restaurant_id);
  SELECT * INTO v_subscription FROM public.subscriptions WHERE restaurant_id = v_restaurant_id ORDER BY updated_at DESC, created_date DESC NULLS LAST LIMIT 1;
  IF v_subscription.id IS NULL THEN RETURN jsonb_build_object('found', false, 'has_erp_access', false, 'status', 'EXPIRED', 'test_mode_enabled', public.erp_subscription_test_mode_enabled(), 'can_manage_billing', v_can_manage_billing); END IF;
  ...
  IF v_status = 'TRIAL' AND (v_subscription.trial_end IS NULL OR v_subscription.trial_end < current_date) THEN
    ...
    INSERT INTO public.subscription_events(subscription_id, restaurant_id, event_type, previous_status, next_status, source) VALUES (v_subscription.id, v_restaurant_id, 'trial_expired', 'TRIAL', 'EXPIRED', 'server_snapshot'); v_status := 'EXPIRED';
  ELSIF v_status = 'ACTIVE' AND v_subscription.current_period_end IS NOT NULL AND v_subscription.current_period_end < current_date THEN
    ...
    INSERT INTO public.subscription_events(subscription_id, restaurant_id, event_type, previous_status, next_status, source) VALUES (v_subscription.id, v_restaurant_id, 'billing_period_elapsed', 'ACTIVE', 'PAST_DUE', 'server_snapshot'); v_status := 'PAST_DUE';
  END IF;
  SELECT * INTO v_plan FROM public.subscription_plans WHERE id = v_subscription.plan;
  SELECT coalesce(jsonb_object_agg(metric, used_amount), '{}'::jsonb) INTO v_usage FROM public.subscription_usage WHERE subscription_id = v_subscription.id AND (period_start = date_trunc('month', current_date)::date OR period_start = date '1970-01-01');
  SELECT id INTO v_pending_payment_id FROM public.subscription_payments WHERE subscription_id = v_subscription.id AND provider IN ('manual_iban', 'mock_test') AND status = 'pending' ORDER BY created_at DESC LIMIT 1;
