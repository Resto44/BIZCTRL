...
CREATE OR REPLACE FUNCTION public.platform_owner_provision(p_user_id uuid, p_mfa_required boolean DEFAULT false)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public SET row_security = off AS $$
BEGIN
  IF auth.role() <> 'service_role' OR p_user_id IS NULL THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'PLATFORM_OWNER_PROVISION_SERVICE_ROLE_ONLY';
  END IF;
  INSERT INTO public.platform_owner_accounts(user_id, status, mfa_required, created_by, updated_at)
  VALUES (p_user_id, 'active', coalesce(p_mfa_required, false), auth.uid(), now())
  ON CONFLICT (user_id) DO UPDATE SET status = 'active', mfa_required = EXCLUDED.mfa_required, updated_at = now();
  RETURN jsonb_build_object('provisioned', true, 'user_id', p_user_id, 'mfa_required', coalesce(p_mfa_required, false));
END;
$$;
...
BEGIN
  ...
  WHERE promotion.plan_id = v_plan.id AND promotion.is_active AND promotion.starts_at <= now() AND promotion.ends_at >= now()
    ...
    AND (NOT promotion.first_time_only OR NOT EXISTS (SELECT 1 FROM public.subscription_payments prior_payment WHERE prior_payment.restaurant_id = v_restaurant_id AND prior_payment.status = 'paid'))
