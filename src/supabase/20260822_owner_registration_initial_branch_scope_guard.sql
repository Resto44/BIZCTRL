...
-- Correct the owner-seed exception so branch-only fields are evaluated only for
-- branch trigger rows. This retains all subscription, capacity, and RLS guards
-- for normal application resource creation.
...
BEGIN
  ...
  -- Owner setup starts from auth.users, where no authenticated browser JWT exists.
  -- Only the nested main branch generated by handle_new_user() can pass this gate.
