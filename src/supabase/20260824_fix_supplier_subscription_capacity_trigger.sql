-- Fix the generic capacity trigger so it can run for suppliers and other
-- tables that do not define branch_key. Existing capacity and scope checks remain unchanged.
...
BEGIN
  ...
  IF TG_TABLE_NAME = 'branches'
     ...
     AND (to_jsonb(NEW) ->> 'branch_key') LIKE 'main-%' THEN
    SELECT EXISTS (
