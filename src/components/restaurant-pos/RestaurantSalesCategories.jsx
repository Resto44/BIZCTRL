import RestaurantCategoryImage,{validCategoryImageUrl} from './RestaurantCategoryImage';
...
export default function RestaurantSalesCategories({tenant,branch,lang='en',close,refresh,onSaved}) {
 ...
 const save=async e=>{
  ...
  const imageUrl=(form.image_url||'').trim();if(!validCategoryImageUrl(imageUrl)){setError(c.invalidImage);return;}
  setBusy(true);setError('');
  try{
   await restaurantRpc('pos_setup',{p_restaurant_id:tenant,p_branch_id:branch,p_command:'category_save',p_payload:{...form,image_url:imageUrl||null,name:form.name.trim(),sort_order:Number(form.sort_order)}});
 ...
 return <Dialog open onOpenChange={open=>{if(!open&&!busy)close();}}><DialogContent dir={lang==='en'?'ltr':'rtl'} className="max-h-[90dvh] overflow-y-auto sm:max-w-xl"><DialogHeader><DialogTitle>{t.manage}</DialogTitle><DialogDescription>{c.hint}</DialogDescription></DialogHeader>
  ...
  {form?<form onSubmit={save} className="space-y-3"><fieldset disabled={busy} className="space-y-3">
   ...
   <label className="grid gap-2 text-sm font-bold">{c.image}<input type="url" dir="ltr" inputMode="url" maxLength={2048} placeholder="https://example.com/category.jpg" className={input} value={form.image_url||''} onChange={e=>set('image_url',e.target.value)}/></label>
   <p className="text-sm text-slate-500">{c.imageHint}</p>
   {form.image_url&&<div className="flex items-center gap-3"><RestaurantCategoryImage src={form.image_url.trim()} className="h-20 w-20 rounded-xl object-cover"/><button type="button" className={button} onClick={()=>set('image_url','')}>{c.removeImage}</button></div>}
   ...
   <button type="button" className={button+' border-blue-600 bg-blue-600 text-white'} disabled={!tenant||!branch||q.isLoading||q.isError} onClick={()=>{setError('');setForm({id:crypto.randomUUID(),name:'',name_ar:'',name_fa:'',image_url:'',sort_order:0,is_active:true});}}><Plus size={18}/>{c.add}</button>
   <div className="space-y-2">{(q.data?.categories||[]).map(row=><article key={row.id} className="flex items-center justify-between gap-3 rounded-xl border p-3"><RestaurantCategoryImage src={row.image_url}/><div className="min-w-0 flex-1"><strong className="break-words">{row['name_'+lang]||row.name}</strong><p className={'mt-1 text-sm '+(row.is_active?'text-emerald-700':'text-slate-500')}>{row.is_active?c.active:c.inactive}</p></div><button type="button" className={button} aria-label={c.edit+' '+row.name} onClick={()=>{setError('');setForm(row);}}><Pencil size={16}/>{c.edit}</button></article>)}</div>
