...
const EnterpriseProductCatalog = memo(function EnterpriseProductCatalog({
  ...
  useEffect(() => {
    if (canImportProductSpreadsheet && openImportSignal > 0) setShowImport(true);
  }, [canImportProductSpreadsheet, openImportSignal]);

  useEffect(() => {
    if (!canImportProductSpreadsheet) setShowImport(false);
  }, [canImportProductSpreadsheet]);
  ...
  const deleteProduct = (row) => {
    if (canDeleteProducts) onDelete?.(row.product_data || row);
  };
    ...
    <div className="min-w-0 space-y-4">
      <section className="overflow-hidden rounded-2xl border border-blue-200 bg-[radial-gradient(circle_at_top_right,rgba(37,99,235,0.14),transparent_46%)] p-4 dark:border-blue-900 sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm text-slate-600 dark:text-slate-300">{canImportProductSpreadsheet ? 'Import once, govern centrally, then activate only the products each branch needs.' : 'Create and govern master products centrally, then activate only the products each branch needs.'}</p>
          </div>
          <div className={cn('grid gap-2 sm:flex', canImportProductSpreadsheet ? 'grid-cols-2' : 'grid-cols-1')}>
            {canImportProductSpreadsheet ? <Button variant="outline" className="h-11 rounded-xl bg-white dark:bg-slate-950" onClick={() => setShowImport(true)}><FileSpreadsheet className="mr-2 h-4 w-4" />Excel Import</Button> : null}
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="ml-auto flex gap-2">
          ...
          {onBarcode && <Button variant="outline" size="sm" className="rounded-lg" onClick={() => onBarcode()}><Barcode className="mr-1.5 h-4 w-4" />Create Barcode</Button>}
      ...
      {catalogQuery.isError ? <CatalogError message={catalogQuery.error?.message} onRetry={() => catalogQuery.refetch()} /> : catalogQuery.isLoading ? (
        ...
        <div className="flex min-h-56 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50/60 p-6 text-center dark:border-slate-700 dark:bg-slate-900/40"><Package className="h-9 w-9 text-blue-600" /><h3 className="mt-3 font-black">No matching master products</h3><p className="mt-1 text-sm text-slate-500">Change the filters{canImportProductSpreadsheet ? ', import an Excel catalog,' : ''} or create a product.</p><div className="mt-4 flex gap-2">{canImportProductSpreadsheet ? <Button variant="outline" onClick={() => setShowImport(true)}><FileSpreadsheet className="mr-2 h-4 w-4" />Import Excel</Button> : null}<Button onClick={onAdd}><Plus className="mr-2 h-4 w-4" />Create Product</Button></div></div>
      ) : (
        <>
      ...
      {canImportProductSpreadsheet ? <ProductBulkImportDialog open={showImport} onOpenChange={setShowImport} restaurantId={restaurantId} selectedBranch={selectedBranch} onImported={refresh} isAllowed={canImportProductSpreadsheet} /> : null}
    </div>
  );
});
