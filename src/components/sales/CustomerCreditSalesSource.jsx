...
export default function CustomerCreditSalesSource({
  ...
  const paymentMax = outstanding + creditSale;
  const after = Math.max(0, outstanding + creditSale - Math.min(payment, paymentMax));
  const saleExceedsLimit = creditLimit > 0 && creditSale > available;
  const paymentExceedsDebt = payment > paymentMax;
  ...
  return (
    <div className={`min-w-0 rounded-xl border bg-white p-3 ${saleExceedsLimit || paymentExceedsDebt ? 'border-red-300' : 'border-slate-200'}`} data-testid="customer-credit-sales-source">
      ...
      <div className="mt-3 space-y-2">
        ...
        <Input
          value={selectedCustomer ? customerName : customerSearch}
          onChange={(event) => {
            if (selectedCustomer) {
        ...
        {matches.length > 0 && !selectedCustomer && (
          <div className="max-h-52 overflow-y-auto rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
            {matches.map((customer) => (
              <button key={customer.id} type="button" className="flex w-full min-w-0 items-center gap-2 rounded-lg px-3 py-2.5 text-left hover:bg-slate-50" onClick={() => selectCustomer(customer)}>
        ...
        {selectedCustomer && (
          <div className="flex min-w-0 items-center gap-3 rounded-xl border border-blue-100 bg-blue-50/60 px-3 py-2.5">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white text-blue-700"><UserCheck className="h-4 w-4" /></div>
            <div className="min-w-0"><div className="truncate text-sm font-bold text-slate-950">{customerName}</div><div className="truncate text-xs text-slate-500" dir="ltr">{customerIdentifier || '—'}</div></div>
          </div>
        )}
      </div>
        ...
        <>
          ...
          <div className="mt-3 grid min-w-0 gap-2 lg:grid-cols-2">
            <div className="min-w-0 rounded-xl border border-slate-200 bg-slate-50/60 p-3">
              <div className="mb-2 flex items-center gap-2"><Plus className="h-4 w-4 shrink-0 text-blue-700" /><div><div className="text-sm font-bold text-slate-900">Credit Sale</div><div className="text-[10px] text-slate-500">Creates a receivable. No cash.</div></div></div>
            ...
            <div className="min-w-0 rounded-xl border border-slate-200 bg-slate-50/60 p-3">
              <div className="mb-2 flex items-center gap-2"><Minus className="h-4 w-4 shrink-0 text-emerald-700" /><div><div className="text-sm font-bold text-slate-900">Debt Payment</div><div className="text-[10px] text-slate-500">Reduces receivable. No sales revenue.</div></div></div>
              <div className="relative"><span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-xs font-semibold text-slate-400" dir="ltr">{currency}</span><Input value={entry.payment_amount ?? ''} onChange={(event) => onUpdate(entry.id, 'payment_amount', event.target.value)} className="h-11 pl-14 text-base tabular-nums" inputMode="decimal" type="text" placeholder="0.00" disabled={disabled || outstanding <= 0 || isRecordingPayment} aria-label="Debt payment amount" /></div>
              {paymentExceedsDebt && <div className="mt-2 text-[10px] font-semibold text-red-700">Payment cannot exceed the receivable.</div>}
            </div>
          </div>

          <div className="mt-2 grid gap-2 sm:grid-cols-[1fr_auto] sm:items-end">
            <div className="min-w-0"><Label className="mb-1 block text-[10px] font-bold uppercase tracking-wide text-slate-500">Payment Method</Label><Select value={entry.payment_method || 'cash'} onValueChange={(value) => onUpdate(entry.id, 'payment_method', value)} disabled={disabled || payment <= 0 || outstanding <= 0 || isRecordingPayment}><SelectTrigger className="h-11 w-full text-base"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="cash">Cash</SelectItem><SelectItem value="card">Card</SelectItem><SelectItem value="bank_transfer">Bank Transfer</SelectItem><SelectItem value="online">Online</SelectItem><SelectItem value="wallet">Wallet</SelectItem></SelectContent></Select></div>
            <Button type="button" variant="outline" className="h-11 w-full sm:w-auto" onClick={() => onRecordPayment?.(entry)} disabled={disabled || isRecordingPayment || payment <= 0 || paymentExceedsDebt}><Minus className="mr-1.5 h-4 w-4" />{isRecordingPayment ? 'Recording…' : 'Record Payment'}</Button>
          </div>
          ...
          {(creditSale > 0 || payment > 0) && (
            <div className="mt-3 border-t border-slate-200 pt-3">
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                ...
                <div className="col-span-2 rounded-lg bg-slate-900 px-3 py-2 sm:col-span-1"><div className="text-[10px] font-semibold uppercase tracking-wide text-slate-300">After Transaction</div><div className="mt-0.5 text-base font-extrabold tabular-nums text-white" dir="ltr">{money(currency, after)}</div></div>
              </div>
            </div>
          )}
        </>
