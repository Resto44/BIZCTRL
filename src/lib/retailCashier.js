...
export function receiptHtml(receipt, lang = 'en', paper = '80mm') {
  ...
  <table><thead><tr><th>Item / الصنف</th><th>Qty / كمية</th><th>Total / إجمالي</th></tr></thead><tbody>${(receipt.lines || []).map(line => `<tr><td><span class="item-name">${e(productName(line, lang))}</span><small class="item-meta">${line.sku ? `${e(line.sku)} · ` : ''}${e(money(line.unit_price, currency))} / ${e(line.unit || 'pc')}</small></td><td>${e(line.quantity)}</td><td>${e(Number(line.line_total).toFixed(2))}</td></tr>`).join('')}</tbody></table>
