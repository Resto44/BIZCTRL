const RETAIL_INVENTORY_REALTIME_TABLES = ['retail_inventory_balances', 'retail_inventory_lots', 'retail_inventory_documents', 'retail_inventory_lines', 'retail_inventory_ledger', 'retail_inventory_warehouses', 'products', 'branch_product_assortments', 'purchase_orders'];
...
function removeChannel(client, channel) {
  ...
  try {
    Promise.resolve(client.removeChannel(channel)).catch((error) => {
      console.error('[Retail Inventory] Realtime cleanup failed', error);
    });
  } catch (error) {
    console.error('[Retail Inventory] Realtime cleanup failed', error);
  }
 ...
 */
export function subscribeRetailInventoryRealtime({ client, queryClient, restaurantId, onChange }) {
  ...
  if (isNew) {
    ...
    const queueRefresh = () => {
      ...
      entry.refreshTimer = setTimeout(() => {
        ...
        queryClients.forEach((cache) => {
          cache.invalidateQueries({ queryKey: ['retail-inventory', restaurantId], exact: false });
          ['inventory', 'erp-master-catalog', 'erp-master-catalog-counts'].forEach((key) => cache.invalidateQueries({ queryKey: [key, restaurantId], exact: false }));
        });
      }, 600);
    };
    ...
    try {
      ...
      entry.channel = client.channel(`retail-inventory-${restaurantId}-${++channelSequence}`);
      RETAIL_INVENTORY_REALTIME_TABLES.forEach((table) => {
        entry.channel.on(
          'postgres_changes',
      ...
      console.error('[Retail Inventory] Realtime subscription failed', error);
    }
  }
